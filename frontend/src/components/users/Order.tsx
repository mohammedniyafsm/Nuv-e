...
function Order() {
  ...
  const getProductInfo = (product: string | ProductInfo) => {
  ...
  const handleCancelOrder = (orderId: string | undefined) => {
    ...
    dispatch(cancelOrder(orderId))
      ...
      .then(() => {
        ...
        dispatch(fetchOrders()); // Refresh orders after cancellation
      })
      .catch((err: any) => {
  ...
  const handleReturnOrder = (orderId: string | undefined) => {
    ...
    dispatch(returnOrder(orderId))
        .unwrap()
        .then(() => {
          dispatch(fetchOrders()); // Refresh orders after return
        })
        .then(() => {
          toast.success("Order canceled successfully!", {
            position: "top-right",
      ...
      .catch((err: any) => {
        alert(`Failed to return order: ${err.message || "Unknown error"}`);
      });
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 border-b border-gray-200 pb-4">
              <div className="flex gap-4 items-center">
                <img
                  ...
                  src={
                    getProductInfo(order.items[0]?.productId)?.images.url ||
                    "/images/placeholder.jpg"
                  }
                  alt={`Order ${order._id} item`}
                  loading="lazy"
                />
                <div>
                  <h2
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`px-4 py-1.5 rounded-full text-sm font-medium ${order.orderStatus
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <img
                        ...
                        src={product.images.url || "/images/placeholder.jpg"}
                        alt={product.name}
                        loading="lazy"
                      />
                      <div>
